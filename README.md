...
- **Non-blocking PR lookup**: on the first visit to a new branch, `gh pr view` runs in a background job so the prompt renders immediately; the PR number appears on the next prompt
